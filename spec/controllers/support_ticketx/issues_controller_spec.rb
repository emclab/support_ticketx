...
module SupportTicketx
  RSpec.describe SupportTicketx::IssuesController, type: :controller do
  routes { SupportTicketx::Engine.routes }  #removed deprecated use_route 
  ...
  before(:each) do
    ...
    before(:each) do
      ...
      session[:user_role_ids] = session[:user_privilege].user_role_ids
    ...
    describe "GET 'update'" do
      it "should return success and redirect" do
        user_access = FactoryGirl.create(:user_access, :action => 'update', :resource =>'support_ticketx_issues', :role_definition_id => @role.id, :rank => 1,
                                         :sql_code => "")
        session[:user_id] = @u.id
        @attributes = {:id => 2, :project_id => 2, :subject => 'test Subject', :description => 'Test Description', :priority => 'Major', :created_on => '02/15/2015', :custom_fields => Array.new([ {:custom_field => {:id => 1, :name => 'app_name', :value => 'dummy'} }, {:custom_field => {:id => 2, :name => 'comments', :value => 'test comment'}}, {:custom_field => {:id => 3, :name => 'contact_name', :value => 'amine'} }, {:custom_field => {:id => 4, :name => 'contact_email', :value => 'amine@abc.com'}}])}
        get 'update', {:id => 2, :issue => @attributes}
